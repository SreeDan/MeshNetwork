...
void MeshRouter::set_on_packet_for_me(OnPacketForMe cb) {
    on_packet_for_me_cb_ = std::move(cb);
}
...
void MeshRouter::push_data_bytes(const std::string &from_peer, const std::string &payload_bytes) {
    ...
    event_queue_.push({RouterEvent::PACKET_RECEIVED, from_peer, std::move(pkt)});
}

void MeshRouter::on_peer_connected(const std::string &peer_id) {
    event_queue_.push({RouterEvent::PEER_CONNECTED, peer_id, {}});
}

void MeshRouter::on_peer_disconnected(const std::string &peer_id) {
    event_queue_.push({RouterEvent::PEER_DISCONNECTED, peer_id, {}});
}
...
void MeshRouter::processing_loop() {
    ...
    while (running_) {
        ...
        auto wait_duration = std::chrono::duration_cast<std::chrono::milliseconds>(next_maintenance_time - now);
        ...
        if (event_opt) {
            ...
            if (event.type == RouterEvent::PEER_CONNECTED) {
            ...
            } else if (event.type == RouterEvent::PEER_DISCONNECTED) {
                Log::info("router", {{"peer_id", event.peer_id}}, "peer disconnected");
                std::unique_lock lock(mu_);
                neighbor_views_.erase(event.peer_id);
                recalculate_forwarding_table_locked();
            } else if (event.type == RouterEvent::PACKET_RECEIVED) {
...
void MeshRouter::route_data_packet(const std::string &immediate_src, const std::string &dest_peer,
    ...
    bool is_only_for_me = dest_peer == self_id_;

    if (is_broadcast || is_only_for_me) {
        bool is_my_own_message = pkt.from_peer_id() == self_id_;

        if (on_packet_for_me_cb_ && !is_my_own_message) {
            on_packet_for_me_cb_(pkt);
        }

        if (is_only_for_me) return;
    }
    ...
    std::vector<std::string> next_hops = determine_next_hop(immediate_src, dest_peer);

    for (const std::string &next_peer_id: next_hops) {
        // Dont send it back to the person who sent it
        if (next_peer_id == immediate_src) continue;
        // Dont send it back to yourself
...
void MeshRouter::run_periodic_maintenance() {
    std::unique_lock lock(mu_);
    auto now = std::chrono::steady_clock::now();
